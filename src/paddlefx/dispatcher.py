...
def convert_annotation_to_type(type_str: str) -> tuple[type[Any], ...]:
    ...
    raise NotImplementedError(f"Cannot find type {type_str} in {search_namespaces}")
...
class Parameter:
    ...
    def from_parameter(parameter: inspect.Parameter) -> Parameter:
        ...
        annotation = (
            parameter.annotation if parameter.annotation != parameter.empty else "Any"
        )
...
class Dispatcher:
    ...
    def register(
        ...
        """
        _parameters = tuple(
            Parameter.from_str(parameter) if isinstance(parameter, str) else parameter
